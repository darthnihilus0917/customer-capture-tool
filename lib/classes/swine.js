                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                        ...
                        if (rowNumber >  1) {
                            if (!row.getCell(14).value.includes("14") && !row.getCell(12).value.includes("POS") 
                                ...
                                const newRowData = [
                                    ...
                                    "-", // / KILO
                                    salesAmount.toFixed(3), // HEAD
                                    row.getCell(12).value, // COMPLETE CUSTOMER NAME
                                    row.getCell(14).value, // DIST. CHANNEL
                                ];
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 1}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            ...
                            const customerValue = `IF(IFERROR(VLOOKUP(E${rowNumber},SOTC_SWINE!A2:C${sotcSheet.lastRow.number},{3},FALSE), TRUE)=TRUE, VLOOKUP(E${rowNumber},SOTC_SWINE!A2:D${sotcSheet.lastRow.number},{4},FALSE), VLOOKUP(E${rowNumber},SOTC_SWINE!A2:C${sotcSheet.lastRow.number},{3},FALSE))`;
                            const addressValue = `IF(IFERROR(VLOOKUP(E${rowNumber},SOTC_SWINE!A2:C${sotcSheet.lastRow.number},{3},FALSE), TRUE)=TRUE, VLOOKUP(E${rowNumber},PICKUP_POULTRY!A2:C${pickupSheet.lastRow.number},{3},FALSE), VLOOKUP(E${rowNumber},SOTC_SWINE!A2:C${sotcSheet.lastRow.number},{3},FALSE))`;
                            if (row.getCell(6).value === 'ONE TIME CUSTOMER' || row.getCell(6).value === 'WALK-IN') {
                                row.getCell(6).value = { formula: `IF(ISBLANK(VLOOKUP(E${rowNumber},SOTC_SWINE!A2:D${sotcSheet.lastRow.number},3,FALSE)),VLOOKUP(E${rowNumber},SOTC_SWINE!A2:D${sotcSheet.lastRow.number},4,FALSE),VLOOKUP(E${sotcSheet.lastRow.number},SOTC_SWINE!A2:D${sotcSheet.lastRow.number},3,FALSE))`};
                            }
                            // END OF COMPLETE CUSTOMER NAME
                            // FARM
                            row.getCell(8).alignment = { horizontal: 'center' }; 
                            row.getCell(8).value = { formula: `VLOOKUP(E${rowNumber},SOTC_SWINE!A2:B${sotcSheet.lastRow.number},{2},FALSE)`};
                            // END OF FARM
                            row.getCell(12).value = { formula: `VLOOKUP(K${rowNumber},SKU_SWINE!C2:F${skuSheet.lastRow.number},{4},FALSE)`}; // CLASS
                            row.getCell(13).alignment = { horizontal: 'right' }; // QTY                            
                            row.getCell(20).alignment = { horizontal: 'right' }; // SALE AMT
                        }
                    });
                    ...
                    sourceSheet.eachRow({ includeEmpty: false, firstRow: 3 }, (row, rowNumber) => {
                        if (rowNumber > 3) {
                            if (row.getCell(7).value !== null) {
                                const newRowData = [
                                    parseInt(row.getCell(4).value), // SAP SO
                                    row.getCell(7).value, // FARM
                                    row.getCell(17).value,  // CUSTOMER NAME
                                    row.getCell(18).value, // CUSTOMER ADDRESS
                                    row.getCell(28).value, // MATERIAL ID
