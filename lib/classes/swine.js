                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 1}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            ...
                            if (row.getCell(6).value === 'ONE TIME CUSTOMER' || row.getCell(6).value === 'WALK-IN') {
                                row.getCell(6).value = { formula: `IF(ISBLANK(VLOOKUP(E${rowNumber},SOTC_SWINE!B2:D${sotcSheet.lastRow.number},3,FALSE)),VLOOKUP(E${rowNumber},SOTC_SWINE!B2:E${sotcSheet.lastRow.number},4,FALSE),VLOOKUP(E${sotcSheet.lastRow.number},SOTC_SWINE!B2:D${sotcSheet.lastRow.number},3,FALSE))`};
                            }
                            ...
                            // CLASS
                            row.getCell(12).value = { formula: `VLOOKUP(K${rowNumber},SKU_SWINE!C2:F${skuSheet.lastRow.number},{4},FALSE)`};
                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    sourceSheet.eachRow({ includeEmpty: false, firstRow: 3 }, (row, rowNumber) => {
                        if (rowNumber > 3) {
                            ...
                            // console.log(newRowData);
                            const hasDonation = newRowData.some(value => isNaN(value) || value.includes('Donations'))
                            if (!hasDonation) {
