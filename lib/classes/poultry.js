                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            ...
                            const finalCustomerValue = `IF(IFERROR(VLOOKUP(E${rowNumber},SOTC_POULTRY!A2:B${sotcSheet.lastRow.number},{2},FALSE), TRUE)=TRUE, VLOOKUP(E${rowNumber},PICKUP_POULTRY!A2:B${pickupSheet.lastRow.number},{2},FALSE), VLOOKUP(E${rowNumber},SOTC_POULTRY!A2:B${sotcSheet.lastRow.number},{2},FALSE))`;
                            if (row.getCell(6).value === 'ONE TIME CUSTOMER') {
                                row.getCell(6).value = { formula: `VLOOKUP(${finalCustomerValue}, CUSTOMERS_POULTRY!A2:B${customerSheet.lastRow.number},{2},FALSE)`}
                            }
