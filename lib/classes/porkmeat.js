...
class Porkmeat {
    ...
    clearOutputDataSheet(workbook) {
        workbook.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(() => {
            const clearsheet = workbook.getWorksheet(`${process.env.CON_SHEET_PORKMEAT}`);
    ...
    removeUnrelatedSheets() {
        ...
        workbook.xlsx.readFile(`${process.env.OUTPUT_FILE_PORKMEAT}`).then(() => {
            workbook.eachSheet(sheet => {
                const sku = `SKU_${process.env.CON_SHEET_PORKMEAT}`;
                const customers = `CUSTOMERS_${process.env.CON_SHEET_PORKMEAT}`;
                const sotc = `SOTC_${process.env.CON_SHEET_PORKMEAT}`;
                const pickup = `PICKUP_${process.env.CON_SHEET_PORKMEAT}`;
    ...
    async generateOutputData() {
        try {
            ...
            if (files.length > 0) {
                ...
                return {
                    isProcessed: false,
                    statusMsg: `NO SAP EXPORTED FILE FOUND!`
                }
            }
        } catch(e) {
            return {
    ...
    async buildSOTC() {
        try {
            ...
            if (files.length > 1) {
                return {
                    isProcessed: false,
                    statusMsg: `Too many SOTC files to process. Only one file is allowed in the folder.`
                }
            }
    ...
    checkFileExists(callback) {
        ...
        function check() {
            fs.access(`${process.env.OUTPUT_FILE_PORKMEAT}`, fs.constants.F_OK, (err) => {
                if (!err) {
                    // File exists
                    callback(null, true);
                } else {
